...
% Roberto Perez-Franco and Claude (Fable 5)
%
...
% symbulator 0.5.18 on PyPI), presenting the staged algorithm of the
% classic calculator versions as the conceptual core. Historical
% claims are drawn from the primary sources preserved in references/:
% the 1999 competition paper (Spanish original and English
% translation), the 2001 website documenting Symbulator Q, the 2014
% unfinished book (Symbulator 6), and the version 9 documentation.
...
This monograph documents the solver's internal logic. A history
chapter traces the program from its 1999 origins on the TI-89 to the
2026 computer-algebra port. The core chapter explains, first in words
and then in thoroughly annotated pseudo-code, how a DC analysis
...
monograph closes with worked exemplars of difficult circuits, and with
the voices of Symbulator's users across more than two decades.
\end{abstract}
...
Tecnol\'ogica de Panam\'a) in Azuero. The project has an unusually
well-documented prehistory~\cite{paper1999}. When circuits are solved
...
Programming began on 30 March 1999, over the Holy Week break: first a
minimal DC analysis accepting only resistors and independent sources;
then, cloned from it, the AC, FD, and TR analyses, while the element
catalog grew to essentially its present form---inductors, capacitors,
op-amps, ideal transformers, mutual inductances, and the six two-port
families. By the author's own accounting, the concept took a week of
thought and consultation, and the program about a hundred hours spread
over ten months~\cite{paper1999}.
...
pseudonym \emph{Th\'evenin}, won first place at the IEEE Student Paper
Contest for Latin America in 2000~\cite{paper1999}. Version~5, also
known as \emph{Symbulator~Q}, served as the author's graduation thesis
in January 2001, and was documented by \emph{The Symbulator Book}, a
complete twelve-chapter reference in Spanish whose first four chapters
were translated to English by users Tim Hutcheson and Doug
Burkett~\cite{website2001}.
...
analysis was invoked as a program call, \code{scs\char`\\dc(...)},
...
the description must name a reference node, no element may short its
own two terminals, and every node must have a conduction path to the
reference.
...
The original documentation describes the same machinery operationally,
in terms of \emph{levels}~\cite{website2001}: the program ``first
generates a set of 1st level equations and a list of 1st level
unknowns''---node voltages, plus the currents that must be solved
simultaneously (those of voltage sources and, in DC, inductors); once
solved, ``2nd level expressions,'' generated during the same pass, are
\emph{evaluated} rather than solved---the locally computable currents;
and finally the ``3rd level expressions''---the powers---are built
from everything before them. Degrees classify answers by what they
are; levels, by how they are obtained. They coincide except at the
seam: a voltage source's current is a second-degree answer that the
calculator had to place in the first-level \emph{solve}, while a
current source's current is first-degree precisely because it never
needs solving at all. Keeping the simultaneous set small was what made
...
\item \textbf{Numbers and symbols}, and arithmetic over them
  ...
  analysis, not Python, and its natural names all work.
...
depends on: a reference node exists; no two-terminal element shorts
its own terminals together; and every node has a conduction path to
...
\ForAll{$e \in \mathcal{E}$ with two terminals}
  \State \textbf{check} $n_1 \ne n_2$
    \Comment{no element shorts itself}
\EndFor
...
resistance. The answers are stored under systematic names, mirroring
the calculator variables (\code{v2}, \code{ir1}, \code{pe},
\code{re}) under which the original stored them.
...
state folded in. Source values are expected already written in $s$ (a
step of amplitude $k$ is $k/s$), with the brace shorthand
\code{\{expr\}} transforming a time-domain value on entry. No third
degree is computed in FD: an $s$-domain power has no established
meaning in the curriculum Symbulator serves, and the original made the
same choice.
...
every answer is carried \emph{back} to time. Both crossings have case
analyses that are easy to get wrong and are worth stating exactly.
...
mode~\cite{website2001}---named for the antelope that leaps over
obstacles rather than running through them---exploits linearity. On
entry, each complicated source value is replaced by a fresh
placeholder symbol (``a nickname''); the system, now small and clean,
is solved; and on the way out, the Laplace transform of the original
value is substituted for the placeholder \emph{in every answer}.
Because the circuit is linear, every answer is linear in the source
values, and the late substitution is exact. The mode was always
automatic---``the (Impala) is always there\ldots you have to do
nothing at all''---waking only when such a source was present.
...
The original \emph{(Expert)} mode was explicitly framed as making ``a
hole in that box'': it displayed the generated first-level equation
set and unknowns list, and let the user read, modify, and keep them
before the solve proceeded~\cite{website2001}. Its companion
\code{solves()} tool---a general simultaneous-equation solver ``based
in the Symbulator core's technology''---already had today's shape:
equations, unknowns in braces, and \emph{conditional equations}
(``when'' values) applied as known substitutions, the direct ancestor
of the \code{conditions} argument.
...
Three helpers round out the toolkit. \code{pr} is the parallel
combination $(\sum_k 1/z_k)^{-1}$ with a dominant-zero rule, available
both as a function and as the \code{[a,b]} value shorthand. \code{pf}
reports the power factor $|\cos(\angle V - \angle I)|$ of a
voltage--current phasor pair, labelled \emph{leading}, \emph{lagging},
or \emph{in phase} by the sign of the angle difference. \code{gain}
computes, from input and output voltage--current pairs, the voltage
gain $A_v = v_2/v_1$, current gain $A_i = i_2/i_1$, power gain
$A_p = \Re(-A_v \overline{A_i})$, and input impedance $Z_i = v_1/i_1$.
...
% NOTE (draft): this chapter awaits two circuits from the author ---
% one to be provided, and the one by Prof. Eliane Boulet. The 1999
% two-stage amplifier below is included as the first exemplar.
...
each solved by a single call.

\section{The two-stage amplifier of 1999}
\label{sec:amplifier}

The circuit that closed the original 1999 paper~\cite{paper1999} was
the author's own final project for his Electronics II course: a
two-stage small-signal amplifier model---source resistance, two RC
loading networks, two voltage-controlled current sources
($0.05\,v_1$ and $0.05\,v_3$), coupling capacitors of picofarads
against load resistors of kilohms---whose gain Bode diagram was
requested over $10^{4}$ to $10^{13}$~rad/s. In Symbulator notation:

\begin{quote}
\code{e1,5,0,vg} \\
\code{r1,5,1,150} \quad \code{r2,1,0,1'k} \quad
\code{cc1,1,0,100'p} \quad \code{cc2,1,2,3'p} \\
\code{jd1,2,0,0.05*v\_1} \quad \code{r3,2,0,2'k} \\
\code{r4,2,3,100} \quad \code{r5,3,0,1'k} \quad
\code{cc3,3,0,100'p} \quad \code{cc4,3,4,3'p} \\
\code{jd2,4,0,0.05*v\_3} \quad \code{r6,4,0,2'k}
\end{quote}
...
plot follows. On the 1999 TI-89 this took three minutes to solve and
another 52 seconds to plot---against ``around half an hour, or more''
...
under a second. The circuit makes a fine exemplar because it exercises
so much at once: dependent sources written exactly as the textbook
writes them, mixed magnitudes across nine orders, and a symbolic
source riding through the whole analysis untouched.
...
\begin{quote}
\code{es,e,0,vs} \quad \code{js,0,d,is} \\
\code{r1,e,m,10} \quad \code{r2,a,e,20} \quad \code{r3,m,0,30} \quad
\code{r4,b,m,40} \\
\code{r5,n,m,50} \quad \code{r6,c,d,60} \quad \code{r7,n,d,70} \\
\code{jd1,a,b,0.2*v\_r7} \quad \code{ed2,c,b,0.1*i\_r5} \\
\code{jd3,n,c,2*i\_r1} \quad \code{ed4,0,n,0.7*v\_r6}
\end{quote}
...
This transient problem, from Prof.\ Eliane Boulet, is a classic of the
two-switch genre. For $t<0$, an 18~V source behind 8~$\Omega$ feeds a
resistive ladder (12~$\Omega$, 18~$\Omega$, 6~$\Omega$) holding two
capacitors ($\tfrac16$~F at node $a$, voltage $v_1$;
$\tfrac13$~F at node $b$, voltage $v$); a waveform source
$10\,e^{-t}\sin(2t+30^{\circ})$~A waits behind an open switch. At
$t=0$ both switches act: the DC source disconnects, the waveform
source connects. \emph{Find $v_1$ for $t>0$.}
...
\begin{quote}
\code{e,s,0,18} \quad \code{r8,s,a,8} \quad
\code{c1,a,0,1/6} \quad \code{r12,a,0,12} \\
\code{r18,a,b,18} \quad \code{r6,b,0,6} \quad \code{c2,b,0,1/3}
\end{quote}
...
\begin{quote}
\code{c1,a,0,1/6,9} \quad \code{r12,a,0,12} \quad
\code{r18,a,b,18} \\
\code{r6,b,0,6} \quad \code{c2,b,0,1/3,9/4} \\
...
continuous across the switching instant, as physics demands. Done by
hand, this problem is an afternoon of Laplace algebra with two initial
conditions threaded through every step; with Symbulator it is two
descriptions and two calls, and the student's attention stays where
Prof.\ Boulet presumably wanted it---on the switching, the steady
states, and the modes.
...
\begin{quote}
\code{ea1,na1,0,100} \quad
\code{eb1,nb1,0,(100$\angle{-120}^{\circ}$)} \quad
\code{ec1,nc1,0,(100$\angle 120^{\circ}$)} \\
\code{raa,na1,na2,1} \quad \code{rbb,nb1,nb2,1} \quad
\code{rcc,nc1,nc2,1} \\
\code{rac,na2,nc2,100+24*pi*j} \quad
\code{rcb,nc2,nb2,100+24*pi*j} \quad
...
\begin{quote}
\code{t,2,3,1,n} \quad \code{r2,3,4,z2} \quad \code{e2,4,0,vs2}
\end{quote}
...
\begin{quote}
\code{l1,1,0,2,1} \quad \code{r1,1,0,3} \\
\code{l2,2,0,2} \quad\ \code{r2,2,0,3} \quad \code{m,l1,l2,1}
\end{quote}
...
\begin{quote}
\code{e,1,0,vs} \quad \code{r12,1,2,1/g1} \quad
\code{r14,1,4,1/g2} \\
\code{r23,2,3,1/g} \quad \code{r34,3,4,1/g} \quad
\code{r2o,2,o,1/g4} \quad \code{r4o,4,o,1/g3} \\
\code{o1,0,2,3} \quad \code{o2,0,4,o}
\end{quote}
...
this shape. Symbolic answers on op-amp networks are, in the authors'
...
Symbulator 9 is built on Python and SymPy, and the authors thank their
