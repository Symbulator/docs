...
Table~\ref{tab:timeline} gathers the chronology this chapter has told
in prose.
...
\code{ped2 = 0},
unknowns \code{vs}, \code{is}---each source's value named as the
drawing labels it\footnote{The name \code{is} is worth a footnote of its
own. To a circuits reader it is the most natural name a source current
could have; to Python it is a reserved word, and version~9 refused it at
first---not by any decision of the simulator's, but because the port's
expression reader parses values through Python's own grammar and let the
language's objection reach the user. The drafting of this monograph is
what exposed the leak, and solver 0.5.19 closed it: a keyword used as a
bare name is now shielded from that parser and becomes an ordinary
symbol, so \code{is} works wherever a name may appear. This example is
where the fix shows. It is written with \code{vs} and \code{is} because
that is how the drawing labels the two sources, and the reader who copies
what they see is no longer told that one of the names is unavailable.}---and the positivity written
directly as \emph{conditions}: \code{vs > 0}, \code{is > 0}.
...
\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{references/2000_ieee_award_colour.jpg}
