...
needs to be — it is the answer with the input left as a symbol.

Poles and zeros are then yours to take: factor the denominator, or hand the
expression to SymPy, which is what the answer already is.
...
may be written in *t* and used in an FD run.
...
else, or when you are checking a hand derivation.
...
filter and an active one differ only by having an `o` in them.
:::
