...
dict(num="3.7", title="Using Voltage Division and Current Division to Solve a Circuit",
     ...
     shows="A current source feeding a ladder of seven resistors; the question wants the "
           "current in one branch, $i_o$ in the 24 Ω resistor, and the voltage across "
           "another, $v_o$ across the 30 Ω. We describe the circuit as it is drawn, "
           "naming the source `j` and the resistors `r1` to `r7` from left to right, and "
           "taking the bottom rail as ground, so $i_o$ is the current through `r7` and "
           "$v_o$ the voltage across `r6`. One run in DC returns every current and "
...
dict(num="4.4", title="Using the Node-Voltage Method with Dependent Sources",
     ...
     shows="A resistive circuit with a dependent voltage source at its far end: its value "
           "is eight times $i_\\phi$, the current through the 5 Ω resistor, and the "
           "question wants the power that resistor dissipates. In Symbulator a dependent "
           "source needs no special element; it is a source whose value names another "
           "answer. We name the 5 Ω resistor `r3`, so its current is `ir3`, and write the "
           "dependent source as `e2` with the value `8*ir3`; we number the nodes 1 to 4 "
           "from the source rightward, with the bottom rail as ground. The power asked for "
...
dict(num="4.7", title="Using the Mesh-Current Method with Dependent Sources",
     ...
     shows="Again a dependent voltage source, this time worth fifteen times $i_\\phi$, the "
           "current in the 20 Ω resistor, and again the question wants one resistor's "
           "power, the 4 Ω's. We name the 20 Ω `r4`, so its current is `ir4` and the "
           "source `e2` has the value `15*ir4`; the 4 Ω we name `r3`, and its power is "
...
dict(num="4.21", title="Calculating the Condition for Maximum Power Transfer",
     ...
     shows="A source and two resistors with a load $R_L$ connected across the second, and "
           "the question is which load draws the most power from those terminals, and how "
           "much. That is a question about the circuit *seen from* the load, so we "
           "describe the source and the two resistors and leave $R_L$ out, calling its "
           "terminals node **2** and ground. The {{card:Find equivalent}} card, with "
...
dict(num="4.8", title="A Special Case in the Mesh-Current Method",
     ...
     shows="A voltage source and a current source feeding three resistors, and the "
           "question wants the current in each of the three branches. We name the sources "
           "`e` and `j` and the resistors `r1` to `r3`, and take the bottom rail as "
           "ground. A current source is written like a voltage source, name, two nodes, "
           "value, and its current flows through it from the first node to the second; the "
           "3 A source's arrow points up from the bottom rail into node 2, so we write it "
           "`j,0,2,3`. We write each resistor's nodes in the direction of the figure's "
           "arrow, so that its current is counted as the book counts it: $i_a$ is `ir1`, "
...
dict(num="4.13", title="Using Special Source Transformation Techniques",
     ...
     shows="Two sources, one of voltage and one of current, and six resistors; the "
           "question wants the voltage across the 100 Ω, and the power each source "
           "develops. We write every element as it stands in the figure, the 125 Ω across "
           "the voltage source and the 10 Ω under the current source included, naming the "
...
dict(num="4.23", title="Using Superposition to Solve a Circuit with Dependent Sources",
     ...
     shows="Two independent sources, two dependent ones and three resistors, and the "
           "question wants the voltage across the 20 Ω. We name the 5 Ω resistor `r1`, "
           "the 20 Ω `r2` and the 10 Ω `r3`. The dependent current source is worth "
           ...
           "give, name the three inner nodes **a**, **b** and **c**, and take the "
...
dict(num="5.1", title="Analyzing an Op Amp Circuit",
     ...
     shows="An op amp with two inputs, $v_a$ into the inverting side through a 25 kΩ "
           "resistor and $v_b$ straight into the non-inverting side, with a 100 kΩ "
           "feedback resistor; the question wants the output for two pairs of input "
           "values, and then the range of one input that keeps the amplifier out of "
           "saturation. An ideal op amp is the element `o`, whose three nodes are its "
           "non-inverting input, its inverting input and its output, in that order. We "
           "choose to write the two inputs as voltage sources with the symbolic values "
           "`va` and `vb` rather than the numbers in the question, so that one run "
           "returns the output as a formula in both, and each part can then be asked of "
           "that formula; we name the sources `ea` and `eb`, the resistors `r1` and `r2`, "
           "and number the nodes from the $v_a$ input, the output being node **3**. "
           "Symbulator's ideal op amp has no supplies, so it reports whatever output the "
           "inputs demand; whether that output is within the ±10 V supplies of the figure "
           "is something we check afterwards."),
]
...
dict(num="5.3", title="Designing a Summing Amplifier",
     ...
     shows="A summing amplifier adds several input voltages, each with its own gain, and "
           ...
           "with $R_f$ fixed at 20 kΩ the gains of 4, 1 and 5 want $R_a$ = 20/4 = 5 kΩ, "
           "$R_b$ = 20/1 = 20 kΩ and $R_c$ = 20/5 = 4 kΩ. That is the design, and the run "
           "checks it. We describe the circuit with those four resistors, which we name "
           "`r1`, `r2`, `r3` and `rf`; we write the three inputs as sources `ea`, `eb` and "
           "`ec` with the symbolic values `va`, `vb` and `vc`, so that the output comes "
           "back as a formula; and we write the op amp `o` with its non-inverting input "
           "at ground, its inverting input at a node we call **n**, and its output at "
           "node **4**."),
...
dict(num="5.3c", title="Designing a Summing Amplifier - part (c), in Expert Mode",
     ...
     shows="The same summing amplifier with the three inputs now given as numbers, and "
           "the question turned round: not the output for a given feedback resistor, but "
           "the largest feedback resistor for which the output stays within the rails. We "
           "describe the circuit as in part (a), the inputs as sources of 2, 3 and −1 V, "
           "and leave the feedback resistor as the symbol `rf` instead of a number, so "
...
dict(num="5.5", title="Designing a Difference Amplifier",
     ...
     shows="A difference amplifier amplifies the difference between its two inputs; the "
           ...
           "$R_c$; one choice, the book's, is $R_a$ = $R_c$ = 1.5 kΩ and $R_b$ = $R_d$ = "
           "12 kΩ. We describe the circuit with those values, naming the resistors `ra` to "
           "`rd` after the book's, write the two inputs as sources with the symbolic "
           "values `va` and `vb` so that the output comes back as a formula, and call the "
...
dict(num="5.7", title="Analyzing a Noninverting-Amplifier Circuit Using a Realistic Op Amp Model",
     ...
     shows="A noninverting amplifier built not round an ideal op amp but round a "
           "realistic model of one: a dependent voltage source with a large but finite "
           "gain, an input resistance between its two inputs, and an output resistance in "
           "series with its output. The question wants the gain of the whole amplifier, "
           "output over source. We write the model as those three ordinary elements: we "
           "call the op amp's two input nodes **p** and **n**, write the input resistance "
           "as `ri` between them, the output resistance as `ro`, and the dependent source "
           "as `ea` with the value `50000*(vp-vn)`, the open-loop gain times the voltage "
           "between the inputs, $A(v_p - v_n)$; the other three resistors are `rg`, `rs` "
           "and `rf`. We leave the source as the symbol `vg`, so that the output comes "
           "back as a multiple of it; the gain is that multiple, which we will read in "
...
dict(num="7.1", title="Determining the Natural Response of an RL Circuit",
     ...
     pre=[dict(
        text="An inductor that has been fed by a current source for a long time, and a "
             "switch that then cuts the source off and leaves the inductor to discharge "
             "through three resistors; the question wants the inductor's current and two "
             "other quantities afterwards, as functions of time. There are two intervals, "
...
dict(num="7.3", title="Determining the Natural Response of an RC Circuit",
     ...
     pre=[dict(
        text="A capacitor charged from one source, then switched over to a set of "
             "resistors to discharge; the question wants its voltage and two other "
             "quantities after the switch moves. Two intervals, two runs. At position x "
...
dict(num="7.5", title="Determining the Step Response of an RL Circuit",
     ...
     pre=[dict(
        text="An inductor that has been carrying a current from one source and is "
             ...
             "as `l,1,0,0.2`, so that its current is counted downward like the book's "
...
dict(num="7.10", title="Determining the Step Response of a Circuit with Magnetically Coupled Coils",
     ...
     shows="Two coils wound on one core, both fed from the same node through one "
           "resistor, and a switch that connects the source at $t$ = 0; the question "
           "wants the source's current, the voltage across the coils and the current in "
           "each, all as functions of time. We write each coil as an inductor line of its "
           "own, `l1` and `l2`, and the coupling between them as the `m` line, which names "
           "the two coils and their mutual inductance, 6 H. No energy is stored, so we "
           "write neither inductor with a fifth field. The switch closing at $t$ = 0 "
           "needs no element: in TR a source with a plain numerical value is a step that "
           "begins at $t$ = 0, which is exactly what closing the switch on the 120 V "
           "source does. We name the source `e` and the resistor `r1`, and set the "
           "analysis to TR. $i_o$ is the current through `r1`, $v_o$ the voltage at node "
...
dict(num="7.11b", title="Analyzing an RL Circuit That Has Sequential Switching (after 35 ms)",
     ...
     booknames={"i_l": "i_L"},
     shows="The third interval of the same problem: switch 2 has opened too, removing the "
           "18 Ω resistor, so the inductor now discharges through the 3 Ω and 6 Ω alone, "
           "and the question wants its current from that moment on. We describe that "
           ...
           "is exact. We measure time from the second switching, as the book does, so "
...
dict(num="7.13", title="Finding the Unbounded Response in an RC Circuit",
     ...
     shows="A charged capacitor across two resistors and a dependent current source "
           "whose value is seven times the current in one of the resistors; the question "
           "wants the capacitor's voltage as a function of time, and then how long it "
           "takes to reach 150 V. The 10 V on the capacitor is given, and we write it "
           "into the capacitor's fifth field. We name the 20 kΩ resistor `r2`, so its "
           "current is `ir2` and the dependent source's value is `7*ir2`; the source's "
           "arrow points up into node 1, so we write it `j,0,1,7*ir2`. We set the "
...
dict(num="8.2", title="Finding the Overdamped Natural Response of a Parallel RLC Circuit",
     ...
     shows="A capacitor, an inductor and a resistor in parallel with no source, holding "
           "an initial voltage and an initial current; the question wants the voltage "
           "across the three as a function of time, and the following example wants the "
           "three branch currents. We write each element as a line between node 1 and "
...
dict(num="8.11", title="Finding the Natural Response of a Series RLC Circuit",
     ...
     shows="A charged capacitor discharging round a loop through an inductor and a "
           "resistor; the question wants the loop current and the capacitor's voltage as "
           "functions of time. We write the capacitor with 100 V as its fifth field, and "
           "the inductor and the resistor after it round the loop, naming the three `c`, "
           "`l` and `r`. We write the inductor's nodes as 2 then 1 on purpose, so that its "
           "current is counted in the direction of the book's arrow for $i$; written the "
           "other way round the answer would come back with its sign reversed. We set the "
           "analysis to TR; $i$ is the current through `l` and $v_C$ the voltage at node "
           "1."),
...
dict(num="8.12", title="Finding the Step Response of a Series RLC Circuit",
     ...
     booknames={"v_3": "v_C"},
     shows="A source switched at $t$ = 0 onto a loop of an inductor, a resistor and a "
           "capacitor, none of them holding any energy; the question wants the capacitor's "
           "voltage as a function of time. In TR a source with a plain numerical value is "
           "a step beginning at $t$ = 0, which is what the switch closing does, so the "
           "switch needs no element. We write the 48 V source and then the three elements "
           "in order round the loop, numbering the nodes 1 to 3, and write the inductor "
           "and the capacitor without a fifth field, since neither stores energy. We set "
           "the analysis to TR; $v_C$ is the voltage at node 3, the capacitor's upper "
           "end."),
]
...
dict(num="9.9", title="Combining Impedances in Series and in Parallel",
     ...
     shows="A sinusoidal current source at 200,000 rad/s feeding a resistor, a capacitor, "
           "and a branch of a resistor and an inductor; the question wants the admittance "
           "the source sees, the voltage across it and the current in the inductive "
           "branch as phasors, and those two as functions of time. The inductor and the "
...
dict(num="9.10", title="Using a Delta-to-Wye Transform in the Frequency Domain",
     ...
     shows="A bridge of five impedances fed by one source, every impedance given in ohms "
           "and several of them complex; the question wants the current in every branch "
           "and the voltage at two nodes. We write each impedance exactly as given, "
...
dict(num="9.12", title="Finding a Thevenin Equivalent in the Frequency Domain",
     ...
     shows="An AC circuit with a dependent source inside it, and the question wants its "
           "Thévenin equivalent, a voltage and an impedance, seen from two terminals. The "
...
dict(num="10.12", title="Finding Maximum Power Transfer in a Circuit with an Ideal Transformer",
     ...
     booknames={"z": "R_L", "pmax": "p_{max}"},
     shows="A source feeding a load through an ideal transformer whose windings share a "
           "node; the question wants the load that draws the most average power and how "
           "much that is. An ideal transformer is the element `t`; because its primary "
           "and secondary here share a node, we write each winding as a bracketed pair of "
           "terminals, `[p,x]` for the primary and `[x,a]` for the secondary, followed by "
           "the turns ratio `[4,1]`, and we name the two resistors after their values, "
           "`r60` and `r20`. We leave the load $R_L$ out of the description and name its "
...
dict(num="11.1", title="Analyzing a Wye-Wye Circuit",
     ...
     shownames={"@Abs(v_pa-v_nn)": "|v_pa - v_nn|", "@Abs(v_pa-v_pb)": "|v_pa - v_pb|",
                "@Abs(v_a)": "|v_a|"},
     expect={"i_rla": 1.92 - 1.44j, "i_rlb": -2.2071 - 0.94277j, "i_rlc": 0.28708 + 2.3828j,
             "@Abs(v_pa-v_nn)": 115.22, "@Abs(v_pa-v_pb)": 199.58, "@Abs(v_a)": 118.90},
     booknames={"i_rla": "I_{aA}", "i_rlb": "I_{bB}", "i_rlc": "I_{cC}",
                "@Abs(v_pa-v_nn)": "|V_{AN}|", "@Abs(v_pa-v_pb)": "|V_{AB}|",
                "@Abs(v_a)": "|V_{An}|"},
     after="The three line currents above have the same magnitude 120° apart, which is "
           "what a balanced circuit gives; the b- and c-phase voltages of parts (c), (d) "
           "and (e) have the same magnitudes as the a-phase ones just read, at −120° and "
           "+120° from them, and typing `Abs(v_pb-v_nn)`, `Abs(v_pb-v_pc)` and `Abs(v_b)` "
           "into {{card:Evaluate}} returns those same three magnitudes.",
     shows="A three-phase generator, a three-phase line and a three-phase load, all "
           "Y-connected and all balanced; the question wants the three line currents and "
           "the phase and line voltages at the load and at the generator. We describe "
           "the whole circuit, one phase at a time: three sources at the generator's "
           "internal nodes, which we call **ga**, **gb** and **gc**, each carrying its "
           "phase in its value, `120*exp(-2j*pi/3)` being 120 V at −120°; then the "
           "generator's, the line's and the load's impedance in each phase, all in ohms, "
...
dict(num="13.2", title="The Natural Response of an RC Circuit",
     ...
     pre=[dict(
        text="The circuit of Example 7.3, a capacitor charged from one source and switched "
     ...
     after="That is the transform of the answer Example 7.3 found in the time domain, "
           "$60e^{-25t}$: for the same description FD returns the transform and TR its "
           "inverse.",
     shows="The second interval is the same description as Example 7.3's second run, "
           "the 100 V in the capacitor's fifth field, with one difference: we set the "
           "analysis to FD instead of TR. FD returns each answer as a function of $s$, "
           "the Laplace transform of the answer in time, with the initial condition "
...
dict(num="13.3", title="The Step Response of an RLC Circuit",
     ...
     booknames={"v_1": "V(s)"},
     shows="A parallel RLC circuit holding an initial current and an initial voltage, "
           "with a constant current source switched on at $t$ = 0; the question wants "
           "the voltage across it, worked in the $s$ domain. In the $s$ domain a constant "
           "$I$ switched on at $t$ = 0 has the transform $I/s$, so we write the 24 mA "
           "source's value as `0.024/s`. We put the two initial conditions the question "
           "gives in the fifth fields of the capacitor and the inductor, 50 V and 29 mA "
           "written 0.029, and name the four elements `j`, `c`, `l` and `r`. We set the "
           "analysis to FD, and the answer is the transform $V(s)$ of the voltage asked "
           "for, the voltage at node 1; choosing TR instead would return its inverse, "
...
dict(num="13.5", title="Analyzing a Circuit with Multiple Meshes",
     ...
     shows="Two loops sharing a resistor, each with an inductor, switched onto a source "
           "at $t$ = 0 with no energy stored; the question wants the two loop currents as "
           "functions of time. No energy is stored, so we write the two inductors without "
           "fifth fields, and the switch closing at $t$ = 0 onto the 336 V source needs "
...
dict(num="13.6", title="Creating a Thevenin Equivalent in the s Domain",
     ...
     booknames={"vth": "V_{Th}", "z": "Z_{Th}"},
     shows="A source switched at $t$ = 0 onto two resistors and an inductor, and the "
           "question wants the Thévenin equivalent of that part of the circuit, seen from "
           "two terminals, as functions of $s$. The 480 V source switched on at $t$ = 0 "
           "is a step, whose transform is $480/s$, so we write its value as `480/s`. We "
           "call the terminals a and b node **a** and ground, and name them to the "
           "{{card:Find equivalent}} card with *Thévenin / Norton* chosen; the card "
           "works in FD as it does in DC and AC, and returns the equivalent's voltage and "
           "impedance as functions of $s$. We leave out the capacitor to the right of "
...
dict(num="13.9", title="Deriving the Transfer Function of a Circuit",
     ...
     units={"@v_2/vg": ""},
     solveq=[
      dict(tag="FD, Solve for the poles",
           text="Part (b): the poles of a transfer function are the values of $s$ that "
                "make its denominator zero, and the zeros those that make its numerator "
                "zero. Both are questions for the {{card:Solve}} card with $s$ as the "
                "unknown, and since a pole may be complex we leave "
                "{{ui:real solutions only}} unticked. The denominator first:",
           press="Press {{btn:Solve equations}}.",
           equations=["s**2+6000*s+25000000=0"], unknowns=["s"], conditions=[],
           real_only=False, expect={"s": [-3000 - 4000j, -3000 + 4000j]}, unit="",
           book={"s": "s"}),
      dict(tag="FD, Solve for the zero", entry=False, boxes=("equations",),
           text="Then the numerator:",
           press="Press {{btn:Solve equations}} again.",
           equations=["s+5000=0"], unknowns=["s"], conditions=[],
           real_only=False, expect={"s": -5000}, unit="", book={"s": "s"}),
     ],
     after="So the transfer function has poles at $s = -3000 \\pm j4000$ and a zero at "
           "$s = -5000$.",
     shows="A source driving a resistor, a capacitor and an inductive branch, with the "
           "capacitor's voltage as the output; the question wants the transfer function "
...
dict(num="13.13", title="A Series Inductor Circuit with an Impulsive Response",
     ...
     after="The $\\delta(t)$ in $v_o$ is an impulse. Opening the switch puts the two "
           ...
           "without an impulse of voltage across it, and the weight of this one, 12, is "
...
dict(num="14.6", title="Designing a Parallel RLC Bandpass Filter",
     ...
     parts=[
      ...
      ("b", "The centre frequency is the $\\omega_0$ of that denominator, "
            "$\\omega_0 = 1/\\sqrt{LC}$."),
      ("c", "The bandwidth is the coefficient of $s$ in the denominator, $\\beta = 1/RC$; "
            "the cutoff frequencies are $\\omega_{c} = \\mp\\beta/2 + "
            "\\sqrt{(\\beta/2)^2 + \\omega_0^2}$, and $Q = \\omega_0/\\beta$."),
     ],
     shows="A series resistor feeding a parallel inductor and capacitor, with the "
           "voltage across the pair as the output; the question wants its transfer "
           "function shown to be a bandpass, its centre frequency, bandwidth and $Q$ in "
           "general, and then the resistor and inductor for a given centre frequency and "
           "bandwidth. Nothing in the question is a number until part (d), so we leave "
           "the three elements as the symbols `R`, `L` and `C` and the source as `vi`, "
           "naming the resistor `rr` to keep its name apart from its value. We set the "
           "analysis to FD; the transfer function is the output voltage, at node 2, "
           "divided by the input, which we will read in the {{card:Evaluate}} card, and "
...
dict(num="18.1", title="Finding the z Parameters of a Two-Port Circuit",
     ...
     shows="A T of three resistors seen as a two-port, and the question wants its z "
           "parameters: the four numbers that relate the two port voltages to the two "
           "port currents. We take port 1 as node **1** with ground and port 2 as node "
           "**2** with ground, write the three resistors between those nodes, naming each "
           "after its value, and name the two ports to the {{card:Find equivalent}} card "
...
dict(num="18.6", title="Analyzing Cascaded Two-Port Circuits",
     ...
     shows="Two identical amplifiers, each known only by its four h parameters, one "
           "feeding the other between a source with an internal resistance and a load; "
           "the question wants the voltage gain of the pair. A two-port known only by its "
           "parameters is an element of its own: since these are h parameters we use the "
           "element `h`, and write the four values after the two nodes as a bracketed "
           "term, `[1000,0.0015,100,0.0001]`, the 100 µS written as 0.0001. We write the "
           "two amplifiers as two such lines, `h1` and `h2`, sharing a node we call "
           "**b**, between the source's 500 Ω and the 10 kΩ load; we leave the source as "
           "the symbol `vg`, so that the output comes back as a multiple of it, and the "
           "gain is that multiple, which we will read in the {{card:Evaluate}} card."),
]
